...
- **Use what is here.** `src/components/` already has Button, Input, Select, Drawer, Badge, and the rest, built on Tremor and Radix. Reach for those before adding a dependency or hand-rolling a control. There is no `Dialog` - `Drawer.tsx` is the Radix (`@radix-ui/react-dialog`)-based dialog primitive; that is what an issue-a-thing form belongs in.
